...
default_capture="$repository_root/captures/failures/wrong_dnn/wrong_dnn_raw.pcap"
capture_file="${1:-$default_capture}"
capture_directory="$(dirname -- "$capture_file")"
capture_filter='sctp port 38412 or udp port 8805'
...
echo "Capturing the wrong-DNN control-plane experiment in the foreground."
echo "Interface: any"
echo "Filter:    SCTP port 38412 or PFCP UDP port 8805"
echo "Output:    $capture_file"
echo "Stop the capture with Ctrl+C after the PDU-session result is visible."
echo
