...
ue_config="$repository_root/configs/failures/wrong_dnn/open5gs-ue-wrong-dnn.yaml"
baseline_config="$repository_root/configs/ueransim/open5gs-ue.yaml"
default_log="$repository_root/captures/failures/wrong_dnn/ue_console_raw.log"
...
if [[ ! -r "$ue_config" || ! -r "$baseline_config" ]]; then
  echo "ERROR: a required wrong-DNN or baseline UE configuration is unreadable." >&2
  exit 1
fi
...
echo "Starting the isolated wrong-DNN UE scenario."
echo "Executable:        $ue_binary"
echo "Failure config:    $ue_config"
echo "Baseline config:   $baseline_config"
echo "Intentional fault: requested DNN internet changed to unsupported"
echo "Raw local log:     $log_file"
echo
echo "Expected boundary:"
echo "  Authentication, NAS security, and registration should succeed."
echo "  PDU-session establishment for DNN unsupported should fail."
echo
echo "Root privileges are required if a UE TUN interface is created."
echo "Stop the UE with Ctrl+C after the session result is visible."
echo
